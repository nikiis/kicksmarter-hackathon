...
The backend runs on MongoDB database and uses nodejs as the backend. It also uses python scripts to parse the data provided by the hackathon organisers into what will be stored onto the database. To install and start MongoDB you can read steps [here](https://www.mongodb.com/docs/manual/tutorial/install-mongodb-on-os-x/), but in short on MacOS these instructions can be done:
...
To store the MongoDB database online one is required to create an account using [MongoDB Atlas](https://www.mongodb.com/atlas/database). You will then be presented with the `MONGODB_URI`. To use online database you must create a file `config/deployment.json`, where you need to have a single entry for now:

```
{
    "MONGODB_URI": "YOUR URI with the password embedded",
}
```

### Setup Backblaze 

To store images you will be required to create a [Backblaze](https://www.backblaze.com/) account and create a new Application Key. Note, when creating an Application Key, the Key ID (aka username) and the Key (aka password) will be shown once only, you won't be able to read it again, so note these down. After this is done, add new entries within the `config/deployment.json`:

```
...
    "B2_APP_KEY_ID": "YOUR ID",
    "B2_APP_KEY": "YOUR KEY"
...
```
...
To host the Web API one is required to create an account with [Render](https://render.com/). Later the steps will be given on how to deploy the app there.

# Parsing Data

Python scripts are used for dealing with the raw information given to us by the `Second Spectrum` and `StatsBomb` providers. These are store inside `python-scripts` folder.
...
-   `frameschunksize` specifies the maximum chunk size that the frames will be split into e.g. 1000 will split 5 _ 60 _ 90 = 27000 frames (assuming FPS of 5 Hz and 90 min game) into 27000 / 1000 = 27 chunks. This needs to be done since MongoDB document has a limited size of up to 16 MB, thus we cannot store the whole game of e.g. 150 MB inside a single document.
...
node -r tsconfig-paths/register -r ts-node/register ./src/utils/pushFramesToDb.ts ./python-scripts/2312213_frames.json 1000#chunk-size
...
To run the backend server one simply needs to do `npm run dev:backend` from the root of the project.
...
```
query {
  frame(gameId: "2312213", frameIdx: 10) {
    frameIdx
    lastTouch
    homePlayers {
      number
